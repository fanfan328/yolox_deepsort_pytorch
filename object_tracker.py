...
class ObjectTracker(object):
  ...
  def tracking_deepsort(self, file, filter_class='person'):
    ...
    if info_obj['box_nums']>0:
        ...
        if(len(bbox_xywh) and len(bbox_xywh)==len(scores)):
        ...
        if(len(outputs)<len(scores) and len(outputs)>0):
          image, self.person_id_array = vis_track(file, outputs, scores, self.person_id_array)
        else: image = file

    return image, outputs
    
  def track_video(self, file, progressbar='X', model='yolox-s', ckpt='yolox_s.pth'):
